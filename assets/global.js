...
document.querySelectorAll('[data-gender-trigger]').forEach(trigger => {
  trigger.addEventListener('click', () => {
    const gender = trigger.dataset.genderTrigger;

    document
      .querySelectorAll('[data-gender-panel]')
      .forEach(panel => {
        panel.classList.toggle(
          'hidden',
          panel.dataset.genderPanel !== gender
        );
      });
  });
});
...
class MenuDrawer extends HTMLElement {
  ...
  closeMenuDrawer(event, elementToFocus = false) {
    ...
    // 🔴 FIX: reset aria-expanded on main summary
    const mainSummary = this.mainDetailsToggle.querySelector('summary');
    if (mainSummary) {
